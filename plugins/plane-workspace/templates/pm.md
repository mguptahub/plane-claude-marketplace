...
Write a memory checkpoint after each major step (scoping, drafting, creating). Mark `status: completed` when done, `status: paused` if interrupted. See `knowledge/plane.md` for the full memory format.
